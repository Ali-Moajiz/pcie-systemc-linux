...
def print_header(packet, data):
    ...
    print('\n' + '='*80)
    print('%s | Process: %s | PID: %d → %d | Socket: %s' % (
        ts, packet.comm.decode(), packet.pid, packet.peer_pid,
        packet.path.decode()))
    print('Data Length: %d bytes (captured: %d bytes)' % (packet.len, len(data)))
    print('='*80)

def is_printable(b):
    """Check if a byte is printable ASCII or common whitespace"""
    return (32 <= b <= 126) or b in (9, 10, 13)  # tab, newline, carriage return

def is_mostly_text(data):
    """Heuristic to determine if data is mostly text"""
    if len(data) == 0:
        return False
    printable_count = sum(1 for b in data if is_printable(b))
    return printable_count / len(data) > 0.7

def smart_decode(data):
    """Try to decode data intelligently"""
    # Try UTF-8 first
    try:
    ...
    try:
        ...
        return text, 'ascii-lossy'
    except:
        pass
    
    # Try latin-1 as fallback (never fails)
    return data.decode('latin-1', errors='replace'), 'latin-1'

def readable_output(cpu, event, size):
    packet, data = parse_event(event, size)
    print_header(packet, data)
    
    if packet.flags & SS_PACKET_F_ERR:
        print('⚠ ERROR: Unable to capture full data')
        return
    
    # Detect if data is mostly text
    if is_mostly_text(data):
        ...
        print('\n🔢 Binary Data (hex + ASCII view):')
        print('-' * 80)
        hex_ascii_print(data)
...
def hex_ascii_print(data):
    """Enhanced hex dump with ASCII side-by-side"""
...
def main(args):
    ...
    print('Monitoring Unix domain socket traffic...', file=sys.stderr)
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(
        description='Dump unix domain socket traffic with enhanced readability')
    parser.add_argument(
    ...
    parser.add_argument(
        '--format', choices=outputs.keys(), default='readable',
        help='output format (default: readable - auto-detects text vs binary)')
    parser.add_argument(
